...
- 機能
  ...
  - 特に二次関数近似において、数式とベジェ曲線式を切り替えるボタン
...
- [数式コピー,曲線複製,複数選択]
   やり方はまだ考察中
- [点の追加]
   ペンでタップすると点が作られる 今はタップは処理無視している
- [数式遊び]
   今は整数または小数のLaTeX形式の数式が表示されている
   例えば、0.333なら1/3とシンプルに表現する
- [チュートリアル実装]
    簡易なモーダルではなく追従するチュートリアルを実装する
- [モバイル版での操作の修正]
  - ペンの太さスライダーの修正
  - カラーピッカー閉じるボタン
  - undo, redoのボタン
- [保存データの調整]
    いつの間にかフルcurveデータを保存してしまっている
    また、あらゆる曲線を追加したので、それ関連のパラメータの有無を調整したい
- [グラフのグリッドの挙動]
    以前から、画面中央を少しずらしただけで副メモリが登場したり
    拡大縮小の挙動を最新なものにしたい
- [命名の変更]
   class="curve-approx-tabs-inner"にある、一次直線近似、折れ線近似、二次ベジェチェーン、円・楕円近似、選択曲線近似の命名を、一次関数、折れ線、二次ベジェ、円・楕円、混合曲線にそれぞれ変更してください。
...
## [v-1.0.3.7] - 2025-10-30
- カラーパレットをモーダルから常時表示するように変更
- カラーパレットのUIを一新
- ツールボタンとキャンバス関連のボタンを分離
- キャンバスが中央にいるときにはホームボタンを非表示にする
- チュートリアル削除クエリ追加
- バージョン通知削除クエリ追加
- ページ飛びアラート削除クエリ追加
- 消しゴムモードの実装
...
- 節点数のデフォルト値の変更
- ハンバーガーメニューのUIを変更
- ハンバーガーメニューを何回も開いていると挙動がおかしくなる点を修正
- 二次Bスプライン近似モードで `maxKnots` を 2〜9 に設定した場合、節点スライダーが描画を更新しない不具合を修正
- 近似設定の最大節点デフォルト値を5へ変更
- 節点調整後に強調線を最新形状へ同期
- 二次Bスプライン平行移動後も節点数を維持
- y=f(x)用点列クレンジング関数の作成 pointClean.js
- 曲線の状態を変更時、正しく保存できていなかったバグを修正
- 新曲線タイプ追加に伴いGraPen Jsonデータの形式を修正
- 近似曲線設定のUIと位置を変更
- 単一一次関数、単一二次ベジェ曲線、単一円・楕円、複合曲線近似の追加
- 一次関数の式をy=ax+bの形式に変更
- スマホ版での挙動を修正

## [v-1.0.3.6] - 2025-10-06
- 数式カードボタン選択処理の小さな修正
- 節点候補preKnotsをoriginalPointsから復元する処理を作成
- preKnotsを保存JSONデータから削除

## [v-1.0.3.5] - 2025-09-22
- 拡張モードボタンの追加、スタイルの小さな変更
- 近似失敗モーダルの説明文を変更、ボタンのスタイルを変更
- 曲線スタイル変更を拡張モード有効時に表示するよう変更
- 二次曲線の節点スライダー履歴、APIの追加
- スライダーイベント時、節点表示状態を保つよう修正
- 数式カード更新の軽微な最適化
- 曲線のバウンディングボックスを得るAPIの作成
- 曲線に飛ぶAPIの作成
↻ 二次曲線近似を最適化（snapだとしてもバラバラのセグメントは変）
- [改] 拡張モード実装中のため、複雑な曲線の近似を判定してしまう

## [v-1.0.3.4] - 2025-09-20
- 数式文字列生成関数の作成
  差分を数値的に挿入、差分をハイライト文字列として挿入が可能
- PRNに関するGraPen APIの作成
- translate表示のみに統一
- UNDO/REDO で再近似を行うシナリオを平行移動処理に変更
- 数式からSVG文字列を作成するメソッドを作成 generateSVGPathFromFormula
- jsonデータロード時、特定データからcurvesデータを再構成する処理を追加
- 曲線データに不必要なjsonファイルの削減
  latexEquations.rpn, strokeDasharray, style, originalPoints, knotsPoints, latexEquations.type, knotsCount, data を削除
- 数式のプルダウンメニューをデフォルトで広げるように変更
- 平行移動時、曲線選択状態を変えないようにする
- SVG path以外のSVGキャンバスを選択したとき曲線を選択解除する

## [v-1.0.3.3] - 2025-09-15
- 平行移動時、元の点列をつかい再近似を行うシナリオを改修
  単にSVGの平行移動と、数式の更新に変更

## [v-1.0.3.2] - 2025-09-14
- 曲線追加のメソッドを位置引数からオブジェクトに変更
- 手書き曲線追加のメソッドを位置引数からオブジェクトに変更

## [v-1.0.3.1] - 2025-05-30
- 二次曲線近似アルゴリズムの軽微な修正
- 選択したアイテムの数式をCtrl+Cでコピー後、Desmosにペーストすると数式が反映される機能の追加
- 折れ線近似の数式が正しく調整できていないのを修正
- スマホ版ヘッダーを設置
- 二次曲線分割のアルゴリズムを修正
- Snapping機能の追加
  ### 追加の意義
  - 数値が細かすぎると扱いにくいため、わかりやすい数値で表示
  - 精度が低くなるのが難点
  ### ボタンの意義
　平行移動機能有効ボタンと同じ立ち位置で全体の曲線アイテムに付与する
  ### 処理について
  有効時に曲線近似を行うと、ドメインによって数値が丸められる
    丸めた後の処理が残り、復元はできない
  二次関数については y = a(x-p)^2+q のp, qを丸める
  一次関数、折れ線関数は節点の位置を丸めて調整する
  
  今後の実装で、Snapの切り替えのシナリオをつける
- 曲線タイプごとの曲線アイコンを追加
