...
// SaveGraphManager.js fetchGraphsFromCloud()
✓　グラフブロックをクリックすると(urlのハッシュを取得して)
✓ どこかにクエリパラメータから読み込む
 FireStoreからbinデータを読み込み
  bin -> jsonに変換
 ✓　既存のimport機能でグラフ復元
 urlをそのハッシュに変更 https://teth-main.github.io/grapen?h=1234567890

// GraphSaveManager.js SaveGraph()
保存ボタンクリック
 ✓ タイトル決めてもらう
  ✓ 既存のSave Json機能でjsonファイル取得
   json -> binに変換
  ✓ ハッシュ文字列作成
  ✓ 既存のSave PNG機能 + 128x128 + 圧縮でサムネイル画像作成
   その組み合わせをGAS or FireStoreに格納指示
  ✓ ウェブストレージに
   ✓ ハッシュ、タイトルを保存
   ✓ メニューグラフ保存画面に表示指示
  ✓ urlをそのハッシュに変更 https://teth-main.github.io/grapen?h=1234567890
